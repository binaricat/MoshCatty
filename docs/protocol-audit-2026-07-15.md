...
- `cargo test --all-targets`：321 项通过、0 项失败；四个需要真实服务端的 live test 默认 ignored，并已再次从本机连接 Ubuntu 官方服务端逐项执行通过；
...
- 最新改动另在本机 Ubuntu 虚拟机的官方 Mosh 1.3.2 服务端重跑四项真实会话，输入去重、远端进程启动时 30×100 尺寸、TAB 画面、缩放、单条 1,446,920 字节压缩指令和 12 秒静默恢复全部通过；与公网 Ubuntu 24.04 / Mosh 1.4.0 的既有结果形成跨版本互通证据。官方服务端只发送一次最终关闭确认后立即退出，因此测试同时核对“收到确认”或“整个远端会话确实结束”，避免把末包丢失误报为远端残留。
