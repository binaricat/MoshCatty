...
- `cargo test --all-targets`：333 项通过、0 项失败；四个需要真实服务端的 live test 默认 ignored，并已在 Ubuntu 内部分别连接官方 1.3.2、1.4.0 服务端逐项执行通过；
...
- Netcatty 完整自动测试以受控并发运行：5,159 项通过、0 项失败、3 项按平台跳过；本轮 SSH 启动、地址交接和会话切换的 54 项定向测试全部通过。
...
| #2121 现象 | 本审计能确认的边界 |
|---|---|
| `no MOSH CONNECT` / 服务端看不到 UDP | 前者发生在 SSH 启动阶段，现已恢复官方 `-n -tt` 并覆盖分段、无换行和退出收尾；后者还可能来自 SSH 与 UDP 对同一多地址域名选择了不同入口。Netcatty 现会交接 SSH 实际地址并保留原主机名退路，MoshCatty 会并行尝试候选并锁定首个合法路径。Ubuntu 连续 50 次启动和真实错误首选地址测试均通过；仍需在报告问题的 Windows 机器上做最终确认 |
