...
在公网官方服务端上，往返约 800 ms、双向各 10% 丢包、乱序和 1% 重复包同时存在时，带空格输入、退格纠错、中文/emoji、长行、窗口缩放和清屏后重画全部通过。另用逐字节记录程序核对了完整输入序列，发送与接收完全一致。追加的大画面测试先覆盖 180×70 多行内容，随后又在 Ubuntu 官方 1.3.2 服务端上把单条压缩指令提高到 1,446,920 字节，确认跨过旧的 1 MiB 门槛后仍能完整恢复；12 秒双向静默黑洞后，客户端会显示最后联系时间，同一会话恢复并继续执行命令后提示自动清除。
...
最后一轮独立补充复查又发现并修复了先前报告遗漏的问题：首个线状态必须已经带真实窗口尺寸、官方合法状态的收发边界是 4 MiB、首次连接超时必须先通知远端关闭、“最后成功往返”必须使用被确认状态的发送时间，以及长时间单向丢确认时需要像官方一样选择更短的发送基线。大状态实测还暴露了系统 UDP 接收队列过小，现已同步扩大。终端画面解释器同时补齐了完整控制序列吞吐、自定义制表位、插入/自动换行/原点模式、反向滚动、光标保存恢复、宽字符末列等行为。安全方面，Unix 进程现在阻止崩溃转储，密钥及派生材料在用完后清理。

最终又把官方 1.4.0 的终端状态机做成确定性差分生成器，在 macOS 和 Ubuntu 各执行 5,000 组、每组 120 次终端操作。该测试发现三种此前单元测试没有覆盖的宽字符问题：写入宽字符续格、从续格开始清除，以及 Ubuntu 与 xterm.js 对 emoji 宽度判断不一致时后续画面左移；最终代码复核又补出两个相邻宽字符相互覆盖的尾格清理。四项均已修复并留下最小回归测试。Netcatty 的 SSH 启动也恢复为官方默认的 `-n -tt` 方式，确保服务端在启动进程退出前排空 `MOSH CONNECT` 行；Ubuntu 连续 50 次启动均收到连接行。

本轮继续发现两项与 #2121 直接相关的边界：当前客户端原先会在一次轮询中无限收空 UDP 队列，持续刷屏或无效数据洪泛可能拖住键盘；Netcatty 也没有像官方启动器一样把 SSH 实际连接的服务器地址交给 UDP 客户端，域名有多个地址时可能把 UDP 发到另一地址。现已把每轮收包设为公平且有硬上限，并让 Netcatty 同时提供 SSH 实际地址和原主机名作为候选，客户端向候选地址启动、收到首个合法状态后锁定有效路径。Ubuntu 官方 1.3.2 和 1.4.0 均完成了真实互通验证。

本轮进一步把官方服务端的连续画面、MoshCatty 重绘和 Netcatty 实际使用的 xterm.js 放在同一条链路对照。新测试又找到并修复三个右边界问题：末格字符后的退格、宽字符覆盖后露出的底色，以及官方认为两格但页面认为一格的表情落在末格时被错误换行。复审又发现 Unicode 版本之间有 88 个宽度差异，已按 Netcatty 固定使用的 Unicode 15 规则补齐，并逐一对照全部 1,112,064 个有效码点，右边界判断为 0 差异。20,000 组不含宽字符的连续/最终画面对照为 0 差异；更严格的完整产品链路又执行 2,000 组、每组 180 次操作，MoshCatty 与官方画面在 xterm.js 上全部一致。对照同时确认，不能把“官方一次性最终画面”直接当作所有宽字符分段画面的判定标准：官方 Display 会省略藏在宽字符续格下的内容，服务端和页面也会对部分字符宽度作出不同判断。客户端无法凭未收到的信息恢复这种理想最终画面，但本轮完整产品链路没有出现用户可见差异。

发布前的硬阻塞仍是 Windows + ConPTY 人工验收和公网 IPv6 近 MTU/黑洞实测；协议和 Ubuntu 官方服务端范围内未再保留已知的高风险差异。
...
| MoshCatty | 修复链起点 `8cb4f53987e584ad9130d6e2edd225ac1f1b01a3`；补充复查固定点 `f2bb6ad7070d4f81ffd5c3592c98c94c75a39950`；最终结果为本次工作树 |
| 实际互通环境 | Ubuntu 24.04.4 LTS、Linux 6.8、系统 `mosh 1.4.0`；Ubuntu 22.04、系统 `mosh 1.3.2`；另用 macOS Homebrew `mosh-server 1.4.0` 做时间戳与心跳定向复现 |
...
| 地址与 locale 交接 | `mosh.pl` 固定 SSH 实际连接地址，避免 DNS 二次解析选到另一入口；同时处理 IPv4/IPv6、locale、远端命令、颜色数和端口范围 | Netcatty 现在从 `SSH_CONNECTION` 取出实际服务器地址，同时保留原主机名作退路；MoshCatty 会尝试全部解析结果并在首个合法状态后锁定有效路径 | **明确兼容（功能子集）** | 多地址、双栈、SSH 别名和公网映射不再只依赖 DNS 的第一个结果；颜色数和高级启动参数仍由 Netcatty 当前界面范围决定 |
| 密钥环境变量与销毁 | 官方读入后立即清除环境变量、禁用 core dump，并在销毁时清理密钥上下文 | 当前清除环境变量；Unix 禁用 core dump；AES key schedule、OCB 派生掩码和临时密钥在销毁时清理 | **明确兼容（Windows 转储策略待验收）** | 会话秘密不再长期留在环境或普通崩溃转储中；Windows 正式安装包仍需检查系统转储策略 |
...
| 收发与重组上限 | 官方单个加密数据报上限 2048 字节，压缩/解压完整指令缓冲为 4 MiB | 收发、压缩体重组与解压结果均以 4 MiB 为上限；分片编号受线格式上限约束；系统 UDP 接收队列按完整大状态扩大 | **明确兼容** | 1–4 MiB 官方合法状态不会再被冻结；超过 4 MiB 或分片号会回绕的状态会明确拒绝；Ubuntu 真机跨过旧 1 MiB 门槛通过 |
...
| ACK 与 throwaway | ACK 只向前，且只能确认发送端仍保存的真实发送状态；接收端在克隆旧基线后再执行 throwaway | 当前相同；从未发送或已经裁掉的状态号确认会忽略 | **明确兼容** | 避免伪造/迟到确认错误裁掉仍需重发的输入 |
| 出站状态窗口 | 官方保留多份发送状态，并用 assumed-receiver / prospective-resend 在增量和可靠重发间选择 | 当前保留最多 32 个在途状态，并按官方时间窗和长度条件选择最近发送基线或已确认基线 | **明确兼容** | 大批输入后继续输入的定向测试确认只发送新增内容；小状态仍按官方条件做可靠累计重发 |
...
| ACK 延迟与空闲包量 | 官方数据 ACK 通常延迟不超过约 100 ms，空心跳 3 秒 | 当前同样使用 100 ms 延迟确认和 3 秒空心跳；8 ms 主循环只负责调度，不会提前确认 | **明确兼容** | 公网官方服务端实测空闲 10 秒上行 4 包，与官方客户端一致 |
...
| 关闭握手 | 官方用 `new_num = UINT64_MAX` 做双向关闭确认，并有限次重试；首次等待 15 秒也先进入该流程 | 当前在 EOF、信号、远端关闭和首次连接超时时执行同样流程 | **明确兼容** | 单向初始故障测试确认不会直接遗留远端 shell；真机验证同时检查最终确认或远端进程确实结束 |
...
| 客户端永久超时 | 官方仅在最初连接超过约 15 秒时进入关闭，并继续收发直到关闭确认或重试结束；已建立会话可持续等待恢复 | 当前行为相同 | **明确兼容** | 单向初始故障关闭测试与 65 秒完全断网恢复测试均通过 |
| 本地轮询延迟 | 官方每次 `select` 循环只收一个网络包，再回到键盘和画面 | 当前 socket 为非阻塞，每轮公平遍历现有路径，但总收包批次有固定上限；外层有 2 ms 短暂让步 | **有意简化（已限界）** | 无效 UDP 洪泛回归确认单轮会主动让出，不再无限拖住键盘；上限仍足以接住官方超过 1 MiB 的连续分片画面 |
...
| HostBytes 指令语义 | 官方 Display 可能输出 Bell、OSC、SGR、光标移动、滚屏区、制表位、插入/换行/原点等模式 | 当前覆盖这些序列，并完整吞掉不支持的 ECMA-48 指令，保持跨分片 CSI/OSC/UTF-8 carry | **明确兼容** | 定向测试覆盖自定义制表位、宽字符末列、反向滚动、光标保存恢复等；Ubuntu 官方服务端的真实 TAB 输出逐行核对通过 |
| 宽字符下的隐藏内容 | 官方 framebuffer 允许在宽字符续格保存不可见内容，但 Display 增量画面不一定发送该内容；服务端和 xterm.js 对部分字符宽度也不同 | 当前按 Netcatty 实际 xterm.js 宽度重建已收到的画面；不猜测官方未发送的隐藏内容 | **上游继承限制** | 连续画面与一次性最终画面不能作为宽字符场景的唯一判据；改用实际产品链路后，2,000 组、共约 36 万次操作与官方画面全部一致 |
...
- `cargo test --all-targets`：337 项通过、0 项失败；四个需要真实服务端的 live test 默认 ignored，并已在 Ubuntu 内部分别连接官方 1.3.2、1.4.0 服务端逐项执行通过；
...
- 最新改动和完整测试工程已放到同一台 Ubuntu 机器，在机器内部依次切换官方 Mosh 1.3.2 和 1.4.0，完整重跑四项真实会话。输入去重、错误候选地址后的自动选路、远端进程启动尺寸、TAB 画面、缩放、超过 1 MiB 的单条压缩指令和 12 秒静默恢复全部通过。官方服务端只发送一次最终关闭确认后立即退出，因此测试同时核对“收到确认”或“整个远端会话确实结束”，避免把末包丢失误报为远端残留。
- 官方 1.4.0 终端状态机在 macOS 和 Ubuntu 各生成 5,000 组确定性画面变化，重建后的 Netcatty 终端逐格一致；另对 Ubuntu emoji 宽度差异做两组 xterm.js 定向验证，宽字符后的清除和后续字符位置均正确。
- 新增连续画面/一次性最终画面对照：Ubuntu 官方状态机下，20,000 组不含宽字符的随机画面为 0 差异。最终又把 MoshCatty 的逐次重绘与官方逐次重绘分别送入 Netcatty 实际 xterm.js，2,000 组、每组 180 次操作为 0 个用户可见差异；三个可由客户端修正的右边界问题均已留下最小回归。
- Netcatty 完整自动测试以受控并发运行：5,161 项通过、0 项失败、3 项按平台跳过；本轮 SSH 启动、地址交接和会话切换的 56 项定向测试全部通过。
...
| 1600×900 大尺寸后恢复到 100×30 | 公网官方服务端确认两次尺寸，随后 TAB 输出和普通命令继续正确；修复前客户端会直接拒绝大尺寸状态 |
| 公网官方服务端四项顺序回归 | 尺寸、预测无重复、大输出断网恢复、静默黑洞恢复 4/4 通过，共 69.02 秒 |
...
- 宽字符重建：官方状态生成器复现了末列换行残影、从续格开始清屏残影和 emoji 后续内容错位；代码复核补出相邻宽字符覆盖的尾格清理；四组最小用例修复后均通过，组合字符与 ZWJ emoji 仍保持完整。
- 控制输入：官方解析器会把 SGR 鼠标报告 `CSI < ... M` 当作一个完整控制序列；修复前预测层会把坐标数字误当成可见输入，修复后分段鼠标报告也不会产生本地字符。
- 本地字符宽度：预测层现在复用与实际显示相同的 Unicode 15 宽度表；macOS 官方客户端视为单列的符号不再被错误跳过预测。
- 大尺寸：删除固定 100,000 单元的过小限制，改为按实际单元内存估算并保留 128 MiB 总历史上限；1600×900 官方服务端实测通过，超过协议字段或单帧预算的尺寸仍会拒绝。
- SSH 语言环境：Netcatty 不再用 `LC_ALL` 强行启动远端服务，而是按官方包装器的固定顺序把 `LANG`、`LANGUAGE` 和各项 `LC_*` 作为候选交给 `mosh-server`；Ubuntu 24.04 上已证明缺失候选语言时仍可保留远端可用的 `C.UTF-8`，避免握手阶段无故失败。

## 与 Netcatty #2121 的对应关系

| #2121 现象 | 本审计能确认的边界 |
|---|---|
| `no MOSH CONNECT` / 服务端看不到 UDP | 前者发生在 SSH 启动阶段，现已恢复官方 `-n -tt` 和完整候选语言语义，并覆盖分段、无换行和退出收尾；后者还可能来自 SSH 与 UDP 对同一多地址域名选择了不同入口。Netcatty 现会交接 SSH 实际地址并保留原主机名退路，MoshCatty 会并行尝试候选并锁定首个合法路径。Ubuntu 连续 50 次启动、缺失语言环境和真实错误首选地址测试均通过；仍需在报告问题的 Windows 机器上做最终确认 |
