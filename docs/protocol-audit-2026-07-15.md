...
提交 `8cb4f53` 针对 #2121 的画面重建方向是正确的；本轮又修复了对照中实际复现的八项核心缺陷，以及协议版本、密钥格式、分片编号、IPv6 最小 MTU、加密用量边界和密钥环境残留。修复后的关键结果如下：
...
在公网官方服务端上，往返约 800 ms、双向各 10% 丢包、乱序和 1% 重复包同时存在时，带空格输入、退格纠错、中文/emoji、长行、窗口缩放和清屏后重画全部通过。另用逐字节记录程序核对了完整输入序列，发送与接收完全一致。

尚未做到与官方客户端逐项完全相同的主要部分是 ECN 拥塞标记处理、`experimental` 预测模式，以及 Windows ConPTY 的最终视觉验收。这些不阻挡本轮 Linux/macOS 与官方服务端互通，但发布前仍应在目标 Windows 页面完成一次人工操作验收。
...
| 方向位 | 64 位序列字最高位表示 `TO_CLIENT`，低 63 位为序列；收到反方向包必须拒绝 | 常量和收包检查与官方一致 | **明确兼容** | 可阻止把发送方向的数据报反射回来重放 |
...
| ECN | 官方读取 ECN，并通过时间戳惩罚让对端降速 | 未读取 ECN | **有意简化** | 基本互通不依赖 ECN；拥塞链路上的自适应行为弱于官方 |
...
| ACK 延迟与空闲包量 | 官方数据 ACK 通常延迟不超过约 100 ms，空心跳 3 秒 | 当前使用非阻塞收包和 8 ms 主循环，空心跳 3 秒 | **明确兼容** | 公网官方服务端实测空闲 10 秒上行 4 包，与官方客户端一致；持续入站流量仍可能影响某一次主循环的具体调度时间，因此不把 8 ms 写成硬上限 |
| 重传调度 | 官方按 `SRTT/2`（20–250 ms）发送新帧，先积极重传，随后按 RTO + ACK delay；维持发送状态队列 | 当前新状态使用相同 20–250 ms 节奏并保留多份在途状态；未确认状态统一按 RTO 重传 | **有意简化** | 连续输入和空闲包量的实测已接近官方；10 秒后的重传阶段和额外 ACK delay 没有逐项复刻 |
| instruction_id | 官方只要发送一条新的线指令就使用新的 ID | 当前每条线指令也分配独立递增 ID | **明确兼容** | 单元测试覆盖同一状态的后续 ACK/重发不会复用旧分片组 ID |
| 分片重组 | 官方同一 ID 可乱序到达；不同 ID 放弃旧的不完整指令 | 当前支持同 ID 乱序，较小旧 ID 直接丢弃，新 ID 清空旧组装 | **明确兼容** | 对官方单调递增 ID 正常；额外的陈旧过滤不改变最新状态结果 |
| 状态队列上限 | 官方接收队列超过约 1024 后节流且不 ACK 无法保存的状态 | 当前上限 256，终端快照另有 128 MiB 总量与尺寸限制 | **有意简化** | 更早保护内存；极端单向长时间并行分支可能更早停收，需把它视为安全容量取舍 |
...
| RTO 范围 | 官方 `SRTT + 4*RTTVAR`，钳在 50–1000 ms | 相同，初始值改为 500 ms | **明确兼容** | 公式和边界一致；初始值差异只影响首轮节奏 |
...
| 自适应显示 | 官方以 `send_interval=SRTT/2` 判断：高于 30 ms 开启、降到 20 ms 且无活跃预测时关闭 | 当前 `Client::send_interval` 做 SRTT/2 并用相同 20/30 ms 阈值 | **明确兼容** | 大致相当于 RTT 超过 60 ms 时显示预测 |
...
| 预测生命周期 | 官方以帧确认和 glitch 机制管理，没有额外 15 秒墙钟过期 | 当前额外设置 15 秒安全过期 | **有意简化** | 极端高延迟或长断网时会更早撤掉本地预测，但不会把未确认字符永久留在屏幕上 |
| 内部历史模型 | 官方每个条件单元保留 `original_contents` 历史，并可有多组 cursor tentative | 当前文档明确采用单 `original_ch` 和单预测光标近似 | **有意简化** | 常见 shell 输入已覆盖；复杂重打、多个光标 epoch 的所有细节不承诺与官方完全相同 |
| 模式集合 | 官方支持 adaptive/always/never/experimental | 当前支持 adaptive/always/never，未知值退回 adaptive | **有意简化** | 不影响默认和 #2121 关注的模式；缺少 experimental |
...
- `cargo test --all-targets`：251 项通过、0 项失败；两个需要人工环境的 live test 按仓库原有设置保持 ignored；
...
| #2121 现象 | 本审计能确认的边界 |
|---|---|
| `no MOSH CONNECT` | 这是 SSH bootstrap/输出解析阶段，发生在 MoshCatty 网络客户端启动前。该次连接没有 UDP 包并不能证明 UDP 客户端没发；它根本尚未获得端口和 key。应在 Netcatty 握手桥继续查 SSH 退出与输出收尾竞争 |
...
2. 在真实 IPv6 1280 MTU 网络跑一次大输出，验证系统错误之外的静默黑洞场景；
3. 如果产品需要与官方完全一致的拥塞行为，再实现 ECN；如果需要完整预测选项，再补 `experimental` 模式。

## 最终判断

MoshCatty 的核心方向可以保留，不需要大范围重写。对照官方实现后发现的核心差异已经修复，并在“本地客户端连接公网 Ubuntu 官方服务端”的真实拓扑下通过了高延迟、丢包、乱序、重复包、长断网和换端口测试。

从协议互通和 Linux/macOS 实测看，本轮结果已经达到可交付水平。剩余风险集中在 Windows 最终显示链路、真实 IPv6 黑洞和官方可选功能，不再是本次已经复现的核心连接与输入缺陷。
