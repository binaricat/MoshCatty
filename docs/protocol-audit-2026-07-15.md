...
| 重传调度 | 官方按 `SRTT/2`（20–250 ms）发送新帧，活动期按 RTO + 100 ms ACK delay 重传；连续 10 秒未收到包后退到 3 秒节奏 | 当前使用相同的发送、延迟确认和两段重传节奏，并保留多份在途状态 | **明确兼容** | 100 ms、10 秒和 3 秒边界均有定向计时测试；连续输入和空闲包量实测接近官方 |
...
- `cargo test --all-targets`：264 项通过、0 项失败；两个需要真实服务端的 live test 默认 ignored，并已用本机到该 Ubuntu 主机的连接单独执行通过；
